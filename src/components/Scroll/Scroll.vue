<template>
  <div ref="ep-scroll-wrapper" class="ui-scroll-wrapper">
    <div class="ui-scroll-content">
      <div ref="listWrapper" class="ui-scroll-list-wrapper">
        <slot>
          
        </slot>
      </div>
      <slot name="pullup" :pullUpLoad="pullUpLoad" :isPullUpLoad="isPullUpLoad">
        <div class="ui-pullup-wrapper" v-if="pullUpLoad">
    ...
    <slot name="pulldown" :pullDownRefresh="pullDownRefresh" :pullDownStyle="pullDownStyle" :beforePullDown="beforePullDown" :isPullingDown="isPullingDown" :bubbleY="bubbleY">
      <div class="ui-pulldown-wrapper" :style="pullDownStyle" v-if="pullDownRefresh">
...
const COMPONENT_NAME = 'run-scroll'
...
export default {
  ...
  methods: {
    initScroll() {
      if (!this.$refs['ep-scroll-wrapper']) {
        return
      }
      this._calculateMinHeight()
      ...
      this.scroll = new BScroll(this.$refs['ep-scroll-wrapper'], options)
    ...
    _calculateMinHeight() {
      if (this.$refs.listWrapper) {
        this.$refs.listWrapper.style.minHeight = this.pullDownRefresh || this.pullUpLoad ? `${this.$refs['ep-scroll-wrapper'].offsetHeight + 1}px` : 0
      }
    },
    _onPullDownRefresh() {
    ...
    _pullDownHandle() {
      ...
      this.$emit(EVENT_PULLING_DOWN,this.forceUpdate)
    },
    ...
    _pullUpHandle() {
      ...
      this.$emit(EVENT_PULLING_UP,this.forceUpdate)
    },
    _reboundPullDown() {
