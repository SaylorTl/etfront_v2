    ...
    <div class="unit-bg page-temp-result">
        <div class="pay-result-card card" v-if="step === 1">
            ...
            <div class="pay-result-btn">
                <x-xbutton @click.native="redirectPage">知道了</x-xbutton>
            </div>
        </div>
...
export default {
    ...
    mounted() {
        ...
        let { status, plate } = this.$route.query;
        if (!!plate) {
            this.plate = plate;
        }
        if (!!status) {
    ...
    methods: {
        savePlate() {
            return utils.gateway(utils.api.addHistoryPlate, { plate: this.plate }).then(result => result);
        },
        timeover() {
            this.redirectPage();
        },
